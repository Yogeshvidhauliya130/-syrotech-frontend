...
// ✅ Indian States list (added same as Dashboard.jsx / CustomerDashboard.jsx)
const INDIAN_STATES = [
...
// ✅ Indian Cities list (added same as Dashboard.jsx / CustomerDashboard.jsx)
const INDIAN_CITIES = [
...
const [myReassignedProductFilter, setMyReassignedProductFilter] = useState("all");
  ...
  // ✅ CHANGE 4: city/state custom input toggle
      ...
      .then(allTicketsData => {
        ...
        if (sameCustomerTicket) {
          ...
          fetch(`${BASE_URL}/tickets/${sameCustomerTicket.id}`, {
            ...
            .then(r => r.json())
            .then(() => {
              setForm({ category: "", model: "", serialNo: "", mac: "", customer: "", email: "", phone: "", city: "", state: "", country: "", pincode: "", description: "", assignTo: currentUser?.name || "", productImage: "", raisedVia: "call" });
              setImagePreview(""); setFormErrors({});
              setCityCustom(false); setStateCustom(false);
...
.filter(t => myReassignedProductFilter === "all" || t.category === myReassignedProductFilter)
  ...
  const getRaisedFromLabel = (ticket) => {
    ...
    if (isCustomerRaised) return { label: "Customer", bg: "#ede9fe", color: "#5b21b6", icon: "👥" };
        ...
        <div style={{ maxWidth: 1100, margin: "28px auto", padding: "0 16px" }}>
          <div className="form-card">
            ...
            <div className="form-grid">
              ...
              {/* ✅ CHANGE 4: City — dropdown with Indian cities + manual input toggle */}
              <div className="form-field">
              ...
              {/* ✅ CHANGE 4: State — dropdown with Indian states + manual input toggle */}
              <div className="form-field">
  ...
  <select value={myRaisedProductFilter} onChange={e => setMyRaisedProductFilter(e.target.value)}
    ...
    {[...new Set(allTickets.filter(t => t.raisedBy === currentUser?.email && t.source === "support").map(t => t.category).filter(Boolean))].map(p => (
...
<button onClick={() => setReassignFilter(p => !p)} style={{
  ...
  <input placeholder="🔍 Search customer, serial, product..."
                ...
                <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, minWidth: 1100, background: "white" }}>
                  <thead>
                    <tr style={{ background: "linear-gradient(135deg, #059669 0%, #10b981 100%)", position: "sticky", top: 0, zIndex: 2 }}>
                      {["Ticket No","Date","Product","Model","Customer","Assigned To","Status","Issue","Actions"].map((h, i) => (
  ...
  <tr style={{ borderBottom: "1px solid #f0ede8", background: idx % 2 === 0 ? "#f0fdf4" : "white", borderLeft: `4px solid ${STATUS_COLOR[s] || "#ccc"}` }}>
                          ...
                          <td style={{ padding: "12px 12px", whiteSpace: "nowrap", borderRight: "1px solid #d1fae5", textAlign: "left" }}>
  <div style={{ fontSize: 12, color: "#374151", fontWeight: 600 }}>{ticket.model || "—"}</div>
</td>
  ...
  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
    ...
    {["open","pending"].includes(ticket.status) && (
      <button onClick={() => setRmaForm(prev => ({ ...prev, [ticket.id]: { ...prev[ticket.id], show: !rmaForm[ticket.id]?.show } }))}
        style={{ background: "#f5f3ff", border: "1.5px solid #c4b5fd", color: "#5b21b6", padding: "5px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 600 }}>
        🔧 RMA
      </button>
    )}
    {!["resolved","rma"].includes(ticket.status) && (
      <button onClick={() => setReassignForm(prev => ({ ...prev, [ticket.id]: { ...prev[ticket.id], show: !reassignForm[ticket.id]?.show } }))}
        style={{ background: "#fff7ed", border: "1.5px solid #fed7aa", color: "#92400e", padding: "5px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 600 }}>
        🔄 Reassign
      </button>
    )}
  </div>
  ...
  <tr key={`resolveform-mr-${ticket.id}`} style={{ background: "#f0fdf4" }}>
    <td colSpan={9} style={{ padding: "16px 20px" }}>
      <div style={{ background: "linear-gradient(135deg,#ecfdf5,#d1fae5)", border: "2px solid #10b981", borderRadius: 12, padding: "18px 20px" }}>
        <div style={{ fontSize: 13, fontWeight: 800, color: "#065f46", marginBottom: 8 }}>✅ Confirm Resolution</div>
        <textarea rows={3} placeholder="Describe what was resolved..."
          ...
          style={{ width: "100%", padding: "10px 12px", border: "2px solid #6ee7b7", borderRadius: 8, fontSize: 12, outline: "none", fontFamily: "inherit", background: "white", resize: "vertical", boxSizing: "border-box" }}
        />
    ...
    <td colSpan={9} style={{ padding: "16px 20px" }}>
      <div style={{ background: "linear-gradient(135deg,#f5f3ff,#ede9fe)", border: "2px solid #7c3aed", borderRadius: 12, padding: "16px 20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <select value={rmaForm[ticket.id]?.centerId || ""} onChange={e => setRmaForm(prev => ({ ...prev, [ticket.id]: { ...prev[ticket.id], centerId: e.target.value } }))}
            style={{ padding: "10px 12px", border: "2px solid #c4b5fd", borderRadius: 8, fontSize: 13, outline: "none", fontFamily: "inherit" }}>
            <option value="">Select RMA center</option>
            {getFilteredRMACenters(rmaCenters, ticket).map(c => <option key={c.id} value={c.id}>{c.name} — {c.city}</option>)}
          </select>
        </div>
...
{reassignForm[ticket.id]?.show && !["resolved","rma"].includes(ticket.status) && (
  <tr key={`reassignform-mr-${ticket.id}`} style={{ background: "#fffdf0" }}>
    <td colSpan={9} style={{ padding: "16px 20px" }}>
      <div style={{ background: "linear-gradient(135deg,#fffbeb,#fef9c3)", border: "2px solid #f59e0b", borderRadius: 12, padding: "16px 20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <select value={reassignForm[ticket.id]?.newPerson || ""} onChange={e => setReassignForm(prev => ({ ...prev, [ticket.id]: { ...prev[ticket.id], newPerson: e.target.value } }))}
            style={{ padding: "10px 12px", border: "2px solid #fcd34d", borderRadius: 8, fontSize: 13, outline: "none", fontFamily: "inherit" }}>
            <option value="">Select specialist</option>
            {getFilteredReassignPersons(allSupportPersons, ticket, currentUser?.name).map(p => (
              <option key={p.email} value={p.name}>{p.name} — {p.city || "—"}</option>
            ))}
          </select>
                ...
                <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, minWidth: 1100, background: "white" }}>
                  <thead>
                    <tr style={{ background: "linear-gradient(135deg, #f59e0b 0%, #d97706 100%)", position: "sticky", top: 0, zIndex: 2 }}>
                     {["Ticket No","Date","Product","Model","Customer","Reassigned To","Reason","Status","Reassigned At"].map((h, i) => (
                        <th key={i} style={{ padding: "12px 12px", fontSize: 10, fontWeight: 800, color: "white", textTransform: "uppercase", letterSpacing: "0.05em", textAlign: "left", borderRight: "1px solid rgba(255,255,255,0.2)", whiteSpace: "nowrap" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                      ...
                      return (
                        <tr key={ticket.id} style={{ borderBottom: "1px solid #fef3c7", background: idx % 2 === 0 ? "#fffbeb" : "white", borderLeft: `4px solid #f59e0b` }}>
                          ...
                          <td style={{ padding: "12px 12px", whiteSpace: "nowrap", borderRight: "1px solid #fde68a", cursor: "pointer", textAlign: "left" }}
                            onClick={() => setProductPopup({ category: ticket.category, model: ticket.model, serialNo: ticket.serialNo, mac: ticket.mac })}>
                            <div style={{ fontWeight: 700, fontSize: 12, color: "#d97706", textDecoration: "underline", textDecorationStyle: "dotted", textDecorationColor: "#fde68a" }}>{ticket.category || "—"}</div>
                          </td>
  ...
  <div style={{ fontSize: 12, color: "#374151", fontWeight: 600 }}>{ticket.model || "—"}</div>
                          ...
                          <td style={{ padding: "12px 12px", whiteSpace: "nowrap", borderRight: "1px solid #fde68a", textAlign: "left" }}>
                            <div style={{ fontSize: 12, fontWeight: 700, color: "#059669" }}>{myEntry?.to || ticket.assignTo || "—"}</div>
                            {(() => {
        ...
        <div style={{ maxWidth: 1200, margin: "28px auto", padding: "0 16px" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <select value={assignedProductFilter} onChange={e => setAssignedProductFilter(e.target.value)}
  ...
  <option value="all">All Products</option>
  {[...new Set(tickets.map(t => t.category).filter(Boolean))].map(p => (
    <option key={p} value={p}>{p}</option>
  ))}
</select>
          ...
          {filtered.length > 0 && (
            <div style={{ overflowX: "scroll", overflowY: "auto", maxHeight: "82vh", borderRadius: 12, border: "1.5px solid #e0d8d0", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
              <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, minWidth: 1200, background: "white" }}>
                <thead>
                  <tr style={{ background: "linear-gradient(135deg, #059669 0%, #10b981 100%)", position: "sticky", top: 0, zIndex: 2 }}>
                   {["Ticket No","Raised From","Raised By","Product / S/N + MAC","Model","Customer / KYC","Issue","Status","Image","Actions"].map((h, i) => (
