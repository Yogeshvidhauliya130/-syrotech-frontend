...
export default function ProductionSupport() {
  ...
  const handleResolve = async (ticketId) => {
    ...
    const updatedHistory = existingHistory.map((entry, i) => {
      if (i === existingHistory.length - 1) {
        return { ...entry, resolvedNotes: rf.notes.trim(), resolvedAt: now, resolvedBy: currentUser?.name || "Nishant Gupta" };
      }
      return entry;
    });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f0f4f8", fontFamily: "DM Sans, sans-serif" }}>
      ...
      {/* Header */}
      <div style={{ background: "linear-gradient(135deg,#065f46,#10b981)", color: "white", padding: "14px 28px", display: "flex", justifyContent: "space-between", alignItems: "center", boxShadow: "0 2px 12px rgba(16,185,129,0.3)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 16 }}>Syrotech Networks — Production Support</div>
            <div style={{ fontSize: 11, opacity: 0.85 }}>
              🏭 {currentUser?.name}
              <span style={{ marginLeft: 8, background: "rgba(255,255,255,0.25)", padding: "1px 8px", borderRadius: 10, fontSize: 10, fontWeight: 700 }}>Production</span>
            </div>
          </div>
        </div>
      ...
      <div style={{ background: "white", borderBottom: "2px solid #e5e7eb", padding: "0 28px", display: "flex", gap: 0 }}>
        {[["tickets", "🏭 My Production Tickets"], ["raise", "➕ Raise Production Ticket"]].map(([key, label]) => (
          <button key={key} onClick={() => setActiveTab(key)} style={{
            padding: "14px 22px", fontSize: 13,
            fontWeight: activeTab === key ? 800 : 500,
            color: activeTab === key ? "#065f46" : "#6b7280",
      ...
      {activeTab === "raise" && (
        <div style={{ maxWidth: 700, margin: "28px auto", padding: "0 16px" }}>
          <RaiseProductionTicket onSuccess={() => setActiveTab("tickets")} />
        </div>
      )}
      ...
      {activeTab === "tickets" && (
        <div style={{ maxWidth: 1200, margin: "28px auto", padding: "0 16px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 24 }}>
            {[
              ["Total Tickets", counts.all,      "🏭", "#10b981", "#ecfdf5"],
          ...
          <div style={{ background: "white", borderRadius: 12, border: "1.5px solid #e0d8d0", padding: "14px 16px", marginBottom: 14, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <input placeholder="🔍 Search category, model, raised by..."
          ...
          {tickets.length === 0 ? (
            <div style={{ textAlign: "center", padding: 60, background: "white", borderRadius: 14, color: "#aaa" }}>
              ...
              <p>No production tickets assigned yet.</p>
            </div>
          ) : (
              ...
              <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, background: "white", minWidth: 1100 }}>
                <thead>
                  <tr style={{ background: "linear-gradient(135deg,#065f46,#10b981)", position: "sticky", top: 0, zIndex: 2 }}>
                    {["Ticket No", "Date", "Category", "Product Details", "Raised By", "Image", "History", "Status", "Status Update", "Action"].map((h, i) => (
                ...
                <tbody>
                  {displayed.map((t, idx) => {
                    ...
                    return (
                      <>
                        <tr key={id} style={{ borderBottom: "1px solid #f0ede8", background: idx % 2 === 0 ? "#f0fdf8" : "white", borderLeft: `4px solid ${STATUS_COLOR[s] || "#ccc"}` }}>
                          ...
                          <td style={{ padding: "10px 12px", borderRight: "1px solid #e0d8d0" }}>
                            <div onClick={() => setProductPopup({ subCategory: t.subCategory, model: t.model, serialNo: t.serialNo, mac: t.mac })}
                              style={{ fontSize: 11, fontWeight: 700, color: "#10b981", cursor: "pointer", textDecoration: "underline", textDecorationStyle: "dotted", whiteSpace: "nowrap" }}>
                              {t.model || "—"}
                            </div>
                          ...
                          <td style={{ padding: "10px 12px" }}>
                            {s !== "resolved" && (
                              <button onClick={() => setResolveForm(p => ({ ...p, [id]: { ...p[id], show: !p[id]?.show } }))}
                                style={{ background: resolveForm[id]?.show ? "#ecfdf5" : "#10b981", color: resolveForm[id]?.show ? "#065f46" : "white", border: resolveForm[id]?.show ? "1.5px solid #6ee7b7" : "none", padding: "5px 12px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 600 }}>
