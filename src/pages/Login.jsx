...
useEffect(() => {
  fetch(`${BASE_URL}/api/sales-persons`)
    .then(r => r.json())
    .then(data => { if (Array.isArray(data)) setSalesPersons(data); })
    .catch(console.error);
}, []);
 ...
 const handleLogin = async () => {
  ...
  const handleSignup = async () => {
   ...
   if (!/^\d{10}$/.test((form.phone || "").replace(/\s/g,""))) { setError("Enter a valid 10-digit phone number."); return; }
    if (selectedRole === "customer" && !form.state) { setError("Please select your state."); return; }
    if (selectedRole === "customer" && !form.city) { setError("Please select your city."); return; }
  ...
  // Send SMS OTP for both customer and sales person
    setLoading(true);
    try {
      const res = await fetch(`${BASE_URL}/api/send-sms-otp`, {
        method: "POST", headers: { "Content-Type": "application/json" },
...
const createAccount = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        setPage("login");
        setSelectedRole("");
        setForm({ name: "", email: "", password: "", phone: "", companyName: "", customerType: "", state: "", city: "" });
      }, 2000);
    } catch {
      setLoading(false);
      setError("Cannot connect to server.");
...
const INDIAN_STATES_LOGIN = Object.keys(STATE_CITY_MAP).sort();
 ...
 return (
  <div className="login-container">
    ...
    <div className="login-left-panel">
      <img src="/banner4.jpg" alt="Syrotech Banner" />
    </div>
      ...
      <div className="login-card">
        ...
        <div className="login-logo-area">
          ...
          <div className="login-brand-text">
            <div className="login-brand-name">Syrotech Networks</div>
          </div>
        </div>
        ...
        {page === "login" && (
          <div className="form-section">
            <p style={{ fontSize:20, fontWeight:900, color:"#ff5a00", textTransform:"uppercase", letterSpacing:"0.1em", margin:"0 0 4px 0" }}>Ticketing Tool</p>
  ...
  <div className="field-group">
    ...
    <select name="customerType" value={form.customerType} onChange={handleChange} className="field-input">
      ...
      <option value="SI Partner">SI Partner</option>
            ...
            {selectedRole === "customer" && (
              <div className="field-group">
                <label className="field-label">State <span style={{ color: "#ff5a00" }}>*</span></label>
                <select name="state" value={form.state} onChange={handleChange} className="field-input">
                  <option value="">-- Select State --</option>
                  {INDIAN_STATES_LOGIN.map(s => (
            ...
            {selectedRole === "customer" && (
              <div className="field-group">
                <label className="field-label">City <span style={{ color: "#ff5a00" }}>*</span></label>
                <select name="city" value={form.city} onChange={handleChange} className="field-input" disabled={!form.state}>
                  <option value="">{form.state ? "-- Select City --" : "Select state first"}</option>
                  {(STATE_CITY_MAP[form.state] || []).map(c => (
  ...
  <div className="field-group">
    <label className="field-label">
      Your Sales Person <span style={{ color: "#ff5a00" }}>*</span>
    </label>
    <select
      name="salesPerson"
    ...
    >
      ...
      <option value="other">❓ Can't find my sales person</option>
    </select>

    {form.salesPerson === "other" && (
      <input
        type="text"
        name="salesPersonOther"
        placeholder="Type your sales person name"
        value={form.salesPersonOther}
        onChange={handleChange}
        className="field-input"
        style={{ marginTop: 8 }}
      />
...
{page === "signupotp" && (
  <div className="form-section">
    <h2 className="form-title">Verify Phone</h2>
    <p className="form-subtitle">
      We sent a 6-digit OTP to <strong>+91 {form.phone}</strong>
      {selectedRole !== "customer" && <span> for Sales Person verification</span>}
    </p>

    <div className="otp-boxes">
      {signupOtpInput.map((val, i) => (
        <input
          key={i} type="text" inputMode="numeric" maxLength={1}
          value={val} className="otp-box"
          onChange={e => {
            const v = e.target.value.replace(/\D/,"");
            const next = [...signupOtpInput]; next[i] = v;
            setSignupOtpInput(next); setError("");
            if (v && i < 5) document.getElementById(`sotp-${i+1}`)?.focus();
          }}
          onKeyDown={e => {
            if (e.key === "Backspace" && !signupOtpInput[i] && i > 0)
    ...
    <button className={`btn-primary ${loading ? "btn-loading" : ""}`}
      onClick={async () => {
        const otp = signupOtpInput.join("");
        if (otp.length < 6) { setError("Please enter complete 6-digit OTP."); return; }
        setLoading(true);
        try {
          const res = await fetch(`${BASE_URL}/api/verify-sms-otp`, {
            method: "POST", headers: { "Content-Type": "application/json" },
          ...
          setLoading(false);
          if (!res.ok) { setError(data.error || "OTP verification failed."); return; }
          setSuccess("Phone verified! Creating account...");
          await createAccount();
        } catch {
          setLoading(false);
          setError("Cannot connect to server.");
    ...
    <p className="switch-text">
      <span className="link"
        onClick={() => { setPage("signup"); setError(""); setSuccess(""); setSignupOtpInput(["","","","","",""]); }}>
        ← Back to Signup
      </span>
    </p>
  </div>
