...
export default function MyRmaTickets({ tickets: allTickets }) {
  ...
  const fetchTickets = () => {
    ...
    fetch(`${BASE_URL}/tickets?raisedBy=${encodeURIComponent(email)}&ticketType=rma&limit=2000`)
      .then(r => r.json())
      .then(data => {
  ...
  useEffect(() => {
    if (allTickets) {
      const mine = allTickets.filter(
        t => t.ticketType === "rma" && t.raisedBy === currentUser?.email
      );
      setTickets(mine);
      setLoading(false);
    } else {
  ...
  const displayed = tickets
    .filter(t => statusFilter === "all" || (t.status || "open").toLowerCase() === statusFilter)
    .filter(t => {
  ...
  return (
    <>
      ...
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div>
            <h2 style={{ fontSize: 20, fontWeight: 800, color: "#374151", margin: 0 }}>🔧 My RMA Tickets</h2>
            <p style={{ fontSize: 13, color: "#6b7280", marginTop: 4 }}>All RMA tickets raised by you</p>
          </div>
        ...
        {tickets.length === 0 ? (
          <div style={{ textAlign: "center", padding: 60, background: "white", borderRadius: 14, color: "#aaa" }}>
            ...
            <p style={{ marginTop: 12 }}>No RMA tickets raised yet.</p>
          </div>
        ) : (
          <>
            {/* Filters */}
            <div style={{ background: "white", borderRadius: 12, border: "1.5px solid #e0d8d0", padding: "14px 16px", marginBottom: 14, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <input placeholder="🔍 Search customer, category, model, serial, phone, city..."
                value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                style={{ flex: 1, minWidth: 200, padding: "8px 14px", borderRadius: 9, border: "1.5px solid #d1d5db", fontSize: 12, outline: "none", fontFamily: "inherit" }} />
              {searchQuery && (
              ...
              <div style={{ marginLeft: "auto", fontSize: 12, color: "#9ca3af" }}>
                Showing <strong style={{ color: "#374151" }}>{displayed.length}</strong> of <strong style={{ color: "#374151" }}>{tickets.length}</strong>
              </div>
            </div>
              ...
              <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, background: "white", minWidth: 1000 }}>
                <thead>
                  <tr style={{ background: "linear-gradient(135deg, #c94500 0%, #ff5a00 100%)", position: "sticky", top: 0, zIndex: 2 }}>
                    {["Ticket No", "Date", "Category", "Product Details", "Customer", "Status", "Image", "History", "Sup. Updates", "Resolved By"].map((h, i) => (
                ...
                <tbody>
                  {displayed.length === 0 ? (
                    <tr><td colSpan={10} style={{ textAlign: "center", padding: 40, color: "#9ca3af" }}>No tickets found.</td></tr>
                  ) : displayed.map((t, idx) => {
                    const s = (t.status || "open").toLowerCase();
                    const ticketId = t.id || t._id;
                    return (
                      <>
                        <tr key={ticketId} style={{ borderBottom: "1px solid #f0ede8", background: idx % 2 === 0 ? "#faf7f4" : "white", borderLeft: `4px solid ${STATUS_COLOR[s] || "#ccc"}` }}>

                          <td style={{ padding: "10px 12px", whiteSpace: "nowrap", borderRight: "1px solid #e0d8d0" }}>
                            <div style={{ fontSize: 11, fontWeight: 800, color: "#ff5a00" }}>#{t.ticketNumber || "—"}</div>
                          </td>
                          ...
                          <td style={{ padding: "10px 12px", borderRight: "1px solid #e0d8d0" }}>
                            <div onClick={() => setProductPopup({ category: t.category, subCategory: t.subCategory, model: t.model, serialNo: t.serialNo, mac: t.mac })}
                              style={{ fontSize: 11, fontWeight: 700, color: "#ff5a00", cursor: "pointer", textDecoration: "underline", textDecorationStyle: "dotted", whiteSpace: "nowrap" }}>
                              {t.model || "—"}
                            </div>
                            ...
                            {s === "resolved" && (() => {
                              ...
                              return (
                                <div onClick={() => {
                                  ...
                                  const newEntry = { description: newIssue.trim(), raisedBy: currentUser?.email, raisedByName: currentUser?.name, raisedAt: new Date().toISOString() };
                                  fetch(`${BASE_URL}/tickets/${ticketId}`, {
                                    method: "PATCH", headers: { "Content-Type": "application/json" },
                                    body: JSON.stringify({
                                      status: "reopened", resolvedAt: null, resolutionNotes: "",
                                      reopenedAt: new Date().toISOString(), reopenCount: (t.reopenCount || 0) + 1,
                                      issueHistory: [...existingHistory, newEntry], description: newIssue.trim(),
                          ...
                          <td style={{ padding: "10px 12px", textAlign: "center", borderRight: "1px solid #e0d8d0" }}>
                            {t.productImage ? (
                              <button onClick={() => setExpandedImage(prev => prev === ticketId ? null : ticketId)}
                                style={{ background: expandedImage === ticketId ? "#dcfce7" : "#f0fdf4", border: "1.5px solid #86efac", borderRadius: 6, padding: "4px 8px", cursor: "pointer", fontSize: 10, fontWeight: 700, color: "#065f46" }}>
                                📷 {expandedImage === ticketId ? "Hide" : "View"}
                              </button>
                          ...
                          <td style={{ padding: "10px 12px" }}>
                            {t.resolvedBy ? (
                              <div>
                                <div style={{ fontSize: 11, fontWeight: 700, color: "#059669" }}>✅ {t.resolvedBy}</div>
                                {t.resolutionNotes && (
                                  <div style={{ fontSize: 10, color: "#6b7280", marginTop: 2, maxWidth: 150, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={t.resolutionNotes}>
                                    {t.resolutionNotes}
                                  </div>
                                )}
                              </div>
                        ...
                        {expandedImage === ticketId && t.productImage && (
                          <tr key={`img-${ticketId}`}>
