      ...
      <div className="tab-header">
        ...
        <div className="tab-stats" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {[
            ...
            <button key={key} onClick={() => setFilter(key)} style={{ padding: "6px 14px", borderRadius: 16, cursor: "pointer", border: filter === key ? `2px solid ${col}` : "1px solid #d1d5db", background: filter === key ? bg : "white", color: filter === key ? col : "#555", fontWeight: filter === key ? 700 : 400, fontSize: 12, whiteSpace: "nowrap" }}>
                ...
                <tr key={ticket.id} style={{ borderBottom: "1px solid #f0ede8", background: idx % 2 === 0 ? "#faf7f4" : "white", borderLeft: `4px solid ${STATUS_COLOR[s] || "#ccc"}` }}>
                  ...
                  <td style={{ padding: "12px 14px" }}>
                    {(() => {
                      ...
                      return (
                        <>
                          <StarRating value={fb.rating !== undefined ? parseInt(fb.rating) : savedRating} onChange={val => setFeedbackData(prev => ({ ...prev, [ticket.id]: { ...prev[ticket.id], rating: val } }))} />
                          {(fb.rating !== undefined || !hasFeedback) && (
                            <button onClick={() => saveFeedback(ticket.id, ticket)} disabled={savingId === ticket.id}
                              style={{ marginTop: 6, background: savingId === ticket.id ? "#94a3b8" : "#1d4ed8", color: "white", border: "none", borderRadius: 6, padding: "4px 10px", fontSize: 11, cursor: "pointer", fontWeight: 600, display: "block" }}>
                              {savingId === ticket.id ? "⏳" : hasFeedback ? "💾 Update" : "💾 Save"}
                            </button>
                          )}
                          {hasFeedback && fb.rating === undefined && <div style={{ fontSize: 10, color: "#10b981", marginTop: 4 }}>✅ {savedRating}/5</div>}
                          {ticket.feedbackComment && <div style={{ fontSize: 10, color: "#6b7280", marginTop: 4, maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>"{ticket.feedbackComment}"</div>}
                        </>
                      );
                    })()}
                  </td>
...
function Performance() {
  ...
  return (
    <div className="tab-content">
                ...
                {agents.map((agent, idx) => {
                  ...
                  return (
                    <tr key={agent} style={{ borderBottom: "1px solid #f0ede8", background: idx % 2 === 0 ? "#faf7f4" : "white", borderLeft: `4px solid ${col}` }}>
                      ...
                      <td style={{ padding: "12px 18px", textAlign: "center" }}>
                        <div style={{ fontSize: 20, fontWeight: 800, color: stats.total > 0 ? (parseInt(resolutionRate) >= 70 ? "#10b981" : parseInt(resolutionRate) >= 40 ? "#f59e0b" : "#ef4444") : "#9ca3af" }}>{resolutionRate}</div>
                        <div style={{ fontSize: 10, color: "#9ca3af", marginTop: 2 }}>{stats.resolved}/{stats.total}</div>
                      </td>
                      <td style={{ padding: "12px 18px", textAlign: "center" }}>
      ...
      {/* ✅ COMPACT CARDS — below table, score removed */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 12, marginBottom: 24 }}>
        {agents.map((agent, i) => {
          const stats  = getAgentStats(agent);
          const colors = ["#ff5a00","#3b82f6","#10b981","#f59e0b","#8b5cf6"];
          ...
          return (
            <div key={agent} style={{ background: "white", borderRadius: 10, padding: "14px 16px", boxShadow: "0 2px 8px rgba(0,0,0,0.07)", borderTop: `3px solid ${col}` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                <div style={{ width: 32, height: 32, borderRadius: "50%", background: col, color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 14, flexShrink: 0 }}>{agent.charAt(0)}</div>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 13, color: "#374151" }}>{agent}</div>
                  <div style={{ fontSize: 10, color: "#9ca3af" }}>{stats.total} tickets · {periodLabel()}</div>
                </div>
              ...
              {stats.rma > 0 && (
                <div style={{ background: "#f5f3ff", borderRadius: 6, padding: "4px 10px", fontSize: 11, color: "#5b21b6", fontWeight: 600, marginBottom: 8 }}>🔧 {stats.rma} RMA</div>
              )}
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#6b7280", marginBottom: 8 }}>
                <span>⏱️ Avg: <strong>{stats.avgHours === "—" ? "—" : `${stats.avgHours}h`}</strong></span>
                <span>⭐ {stats.avgFeedback === "—" ? "—" : `${stats.avgFeedback}/5`}</span>
              ...
              {stats.resolved > 0 && (
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "#9ca3af", marginBottom: 3 }}>
                    <span>24hr SLA</span>
                    <span style={{ fontWeight: 700, color: stats.compliance >= 80 ? "#10b981" : stats.compliance >= 50 ? "#f59e0b" : "#ef4444" }}>{stats.compliance}%</span>
                  </div>
                  <div style={{ background: "#f3f4f6", borderRadius: 4, height: 5, overflow: "hidden" }}>
                    <div style={{ height: "100%", borderRadius: 4, width: `${stats.compliance}%`, background: stats.compliance >= 80 ? "#10b981" : stats.compliance >= 50 ? "#f59e0b" : "#ef4444" }} />
                  </div>
