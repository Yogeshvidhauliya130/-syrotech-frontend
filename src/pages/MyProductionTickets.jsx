...
export default function MyProductionTickets({ tickets: allTickets }) {
      ...
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: "#6b7280", marginTop: 4 }}>All production tickets raised by you</p>
          </div>
        ...
        {tickets.length === 0 ? (
          <div style={{ textAlign: "center", padding: 60, background: "white", borderRadius: 14, color: "#aaa" }}>
            ...
            <p style={{ marginTop: 12 }}>No production tickets raised yet.</p>
          </div>
        ) : (
          <>
            {/* Filters */}
            <div style={{ background: "white", borderRadius: 12, border: "1.5px solid #e0d8d0", padding: "14px 16px", marginBottom: 14, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <input placeholder="🔍 Search category, model, serial..."
              ...
              <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, background: "white", minWidth: 900 }}>
                <thead>
                  <tr style={{ background: "linear-gradient(135deg, #c94500 0%, #ff5a00 100%)", position: "sticky", top: 0, zIndex: 2 }}>
                    {["Ticket No", "Date", "Category", "Product Details", "Status", "Image", "History", "Sup. Updates", "Resolved By"].map((h, i) => (
                ...
                <tbody>
                  {displayed.length === 0 ? (
                    <tr><td colSpan={9} style={{ textAlign: "center", padding: 40, color: "#9ca3af" }}>No tickets found.</td></tr>
                  ) : displayed.map((t, idx) => {
                    const s = (t.status || "open").toLowerCase();
                    return (
                      <>
                        <tr key={t.id || t._id} style={{ borderBottom: "1px solid #f0ede8", background: idx % 2 === 0 ? "#faf7f4" : "white", borderLeft: `4px solid ${STATUS_COLOR[s] || "#ccc"}` }}>
                          ...
                          <td style={{ padding: "10px 12px", textAlign: "center", borderRight: "1px solid #e0d8d0" }}>
                            {t.productImage ? (
                              <button onClick={() => setExpandedImage(prev => prev === (t.id || t._id) ? null : (t.id || t._id))}
                                style={{ background: expandedImage === (t.id || t._id) ? "#dcfce7" : "#f0fdf4", border: "1.5px solid #86efac", borderRadius: 6, padding: "4px 8px", cursor: "pointer", fontSize: 10, fontWeight: 700, color: "#065f46" }}>
                                📷 {expandedImage === (t.id || t._id) ? "Hide" : "View"}
                              </button>
                        ...
                        {/* Expanded Image Row */}
                        {expandedImage === (t.id || t._id) && t.productImage && (
                          <tr key={`img-${t.id || t._id}`}>
