...
export default function SLAReport() {
  ...
  const productData = useMemo(() => {
    ...
    return Object.entries(map)
      .map(([name, v]) => ({ name, ...v, resRate: pct(v.resolved, v.resolved + (v.open || 0)) }))
  ...
  const cityData = useMemo(() => {
    ...
    return Object.entries(map)
      .map(([city, v]) => ({ city, ...v, resRate: pct(v.resolved, v.resolved + (v.open || 0)) }))
      .sort((a,b) => b.total - a.total)
      .slice(0, 15);
  }, [filtered]);
  ...
  const salesData = useMemo(() => {
    ...
    filtered.forEach(t => {
      ...
      map[name].source = source; // last wins fine
    });
    return Object.values(map)
      .map(v => ({ ...v,resRate: pct(v.resolved, v.resolved + (v.open || 0)) }))
      .sort((a,b) => b.total - a.total);
  }, [filtered]);
  ...
  const sourceBreakdown = useMemo(() => {
    ...
    const sales    = filtered.filter(t => !t.source || (t.source !== "customer" && t.source !== "support")).length;
    return [
        ...
        <div className="sla-section">
          ...
          <div className="sla-kpi-grid">
            {[
              ...
              { label: "Resolved",         value: kpi.resolved,    icon: "✅", color: "#10b981", bg: "#ecfdf5",  sub: `${pct(kpi.resolved, kpi.resolved + kpi.open)}% resolution rate` },
              ...
              <>
                ...
                <div className="sla-sales-grid sla-card-scroll">
                  {salesData.slice(0, 6).map((s, i) => {
                    ...
                    const sourceLabel = s.source === "customer" ? "Customer" : s.source === "support" ? "Support" : "Sales";
                    return (
                ...
                <div className="sla-scroll" style={{ marginTop: 20 }}>
                  <table className="sla-table">
                    ...
                    <tbody>
                      {salesData.map((s, i) => {
                        const srcMap = { customer: { label:"Customer", color:"#7c3aed", bg:"#f5f3ff" }, support: { label:"Support", color:"#059669", bg:"#ecfdf5" } };
