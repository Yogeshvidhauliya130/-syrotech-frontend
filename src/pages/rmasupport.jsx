...
export default function RmaSupport() {
  ...
  const fetchTickets = () => {
    fetch(`${BASE_URL}/tickets?assignTo=${encodeURIComponent("Ravi Kumar")}&ticketType=rma&limit=2000`)
      .then(r => r.json())
      .then(data => {
  ...
  const handleResolve = async (ticketId) => {
    ...
    const updatedHistory = existingHistory.map((entry, i) => {
      if (i === existingHistory.length - 1) {
        return { ...entry, resolvedNotes: rf.notes.trim(), resolvedAt: now, resolvedBy: currentUser?.name || "Ravi Kumar" };
      }
      return entry;
    });
    ...
    <div style={{ minHeight: "100vh", background: "#f0f4f8", fontFamily: "DM Sans, sans-serif" }}>
      ...
      {productPopup && (
        <div onClick={() => setProductPopup(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "white", borderRadius: 14, padding: "24px 28px", maxWidth: 420, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.3)", border: "2px solid #ddd6fe" }}>
            ...
            <div style={{ background: "#f5f3ff", borderRadius: 10, padding: "14px 16px", border: "1px solid #ddd6fe" }}>
              {[["📂 Sub Category", productPopup.subCategory], ["📐 Model", productPopup.model], ["🔢 Serial No", productPopup.serialNo], ["📡 MAC Address", productPopup.mac]].map(([label, val]) => (
        ...
        <div onClick={() => setCustomerPopup(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "white", borderRadius: 14, padding: "24px 28px", maxWidth: 420, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.3)", border: "2px solid #bfdbfe" }}>
            ...
            <div style={{ background: "#eff6ff", borderRadius: 10, padding: "14px 16px", border: "1px solid #bfdbfe" }}>
              {[["👤 Name", customerPopup.customer], ["📞 Phone", customerPopup.phone], ["✉️ Email", customerPopup.email], ["🏙️ City", customerPopup.city], ["🗺️ State", customerPopup.state], ["🌍 Country", customerPopup.country], ["🏢 Company", customerPopup.companyName]].map(([label, val]) => (
                <div key={label} style={{ display: "flex", gap: 10, marginBottom: 10 }}>
      ...
      {statusUpdateForm?.show && (
        <div onClick={() => setStatusUpdateForm({})} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "white", borderRadius: 14, padding: "28px 32px", maxWidth: 560, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.3)", border: "2px solid #ddd6fe" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <div style={{ fontSize: 16, fontWeight: 800, color: "#5b21b6" }}>📝 Status Update</div>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#374151", marginBottom: 8 }}>Status <span style={{ color: "#ef4444" }}>*</span></div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <textarea rows={5} placeholder="Describe the current status..."
                ...
                style={{ width: "100%", padding: "11px 14px", border: "2px solid #ddd6fe", borderRadius: 10, fontSize: 13, fontFamily: "inherit", resize: "vertical", outline: "none", boxSizing: "border-box", color: "#111", lineHeight: 1.6 }} />
            </div>
      ...
      {/* Header */}
      <div style={{ background: "linear-gradient(135deg,#5b21b6,#7c3aed)", color: "white", padding: "14px 28px", display: "flex", justifyContent: "space-between", alignItems: "center", boxShadow: "0 2px 12px rgba(124,58,237,0.3)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 16 }}>Syrotech Networks — RMA Support</div>
            <div style={{ fontSize: 11, opacity: 0.85 }}>
              🔧 {currentUser?.name}
              <span style={{ marginLeft: 8, background: "rgba(255,255,255,0.25)", padding: "1px 8px", borderRadius: 10, fontSize: 10, fontWeight: 700 }}>RMA</span>
            </div>
          </div>
        </div>
      ...
      <div style={{ background: "white", borderBottom: "2px solid #e5e7eb", padding: "0 28px", display: "flex", gap: 0 }}>
        {[["tickets", "🔧 My RMA Tickets"], ["raise", "➕ Raise RMA Ticket"]].map(([key, label]) => (
          <button key={key} onClick={() => setActiveTab(key)} style={{
            padding: "14px 22px", fontSize: 13,
            fontWeight: activeTab === key ? 800 : 500,
            color: activeTab === key ? "#5b21b6" : "#6b7280",
      ...
      {/* Tickets Tab */}
      {activeTab === "tickets" && (
        <div style={{ maxWidth: 1300, margin: "28px auto", padding: "0 16px" }}>

          {/* Stats */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 24 }}>
            {[
              ["Total Tickets", counts.all,      "🔧", "#7c3aed", "#f5f3ff"],
          ...
          <div style={{ background: "white", borderRadius: 12, border: "1.5px solid #e0d8d0", padding: "14px 16px", marginBottom: 14, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
            ...
            {[["all", "All"], ["open", "🔓 Open"], ["resolved", "✅ Resolved"], ["reopened", "🔄 Reopened"]].map(([key, label]) => (
              <button key={key} onClick={() => setStatusFilter(key)} style={{
                ...
                {label}
                <span style={{ marginLeft: 4, fontSize: 10, fontWeight: 700, background: statusFilter === key ? (STATUS_COLOR[key] || "#374151") : "#e5e7eb", color: statusFilter === key ? "white" : "#555", borderRadius: 10, padding: "1px 6px" }}>
                  {counts[key] ?? 0}
                </span>
              </button>
            ))}
            ...
            <select value={dateSort} onChange={e => setDateSort(e.target.value)} style={{ padding: "6px 12px", borderRadius: 8, border: "1.5px solid #d1d5db", fontSize: 12, outline: "none", fontFamily: "inherit" }}>
              <option value="newest">Newest First ↓</option>
              <option value="oldest">Oldest First ↑</option>
            </select>

            <input placeholder="🔍 Search category, model, customer, raised by..."
              value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
              style={{ flex: 1, minWidth: 220, padding: "8px 14px", borderRadius: 9, border: "1.5px solid #d1d5db", fontSize: 12, outline: "none", fontFamily: "inherit" }} />
            {searchQuery && (
              <button onClick={() => setSearchQuery("")} style={{ background: "#f3f4f6", border: "1px solid #d1d5db", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontSize: 12, color: "#6b7280" }}>✕</button>
            )}
          </div>
          ...
          {tickets.length === 0 ? (
            <div style={{ textAlign: "center", padding: 60, background: "white", borderRadius: 14, color: "#aaa" }}>
              ...
              <p>No RMA tickets assigned yet.</p>
            </div>
          ) : (
            <div style={{ overflowX: "scroll", overflowY: "auto", maxHeight: "72vh", borderRadius: 12, border: "1.5px solid #e0d8d0", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
              <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, background: "white", minWidth: 1200 }}>
                <thead>
                  <tr style={{ background: "linear-gradient(135deg,#5b21b6,#7c3aed)", position: "sticky", top: 0, zIndex: 2 }}>
                ...
                <tbody>
                  {displayed.map((t, idx) => {
                    ...
                    return (
                      <>
                        <tr key={id} style={{ borderBottom: "1px solid #f0ede8", background: idx % 2 === 0 ? "#faf5ff" : "white", borderLeft: `4px solid ${STATUS_COLOR[s] || "#ccc"}` }}>

                          {/* Ticket No */}
                          <td style={{ padding: "10px 12px", whiteSpace: "nowrap", borderRight: "1px solid #e0d8d0" }}>
                            <div style={{ fontSize: 11, fontWeight: 800, color: "#7c3aed" }}>#{t.ticketNumber || "—"}</div>
                          </td>
                          ...
                          {/* Product Details */}
                          <td style={{ padding: "10px 12px", borderRight: "1px solid #e0d8d0" }}>
                            <div onClick={() => setProductPopup({ subCategory: t.subCategory, model: t.model, serialNo: t.serialNo, mac: t.mac })}
                              style={{ fontSize: 11, fontWeight: 700, color: "#7c3aed", cursor: "pointer", textDecoration: "underline", textDecorationStyle: "dotted", whiteSpace: "nowrap" }}>
                              {t.model || "—"}
                            </div>
                          ...
                          {/* Customer */}
                          <td style={{ padding: "10px 12px", borderRight: "1px solid #e0d8d0" }}>
                            <div onClick={() => setCustomerPopup({ customer: t.customer, phone: t.phone, email: t.email, city: t.city, state: t.state, country: t.country, companyName: t.companyName })}
                              style={{ fontSize: 12, fontWeight: 700, color: "#1d4ed8", cursor: "pointer", textDecoration: "underline", textDecorationStyle: "dotted", whiteSpace: "nowrap" }}>
                              {t.customer || "—"}
                            </div>
                          </td>
                          ...
                          {/* Raised By */}
                          <td style={{ padding: "10px 12px", borderRight: "1px solid #e0d8d0" }}>
                            <div onClick={() => setRaisedByPopup({ name: t.raisedByName, email: t.raisedBy, role: "Sales" })}
                              style={{ fontSize: 11, fontWeight: 700, color: "#059669", cursor: "pointer", textDecoration: "underline", textDecorationStyle: "dotted", whiteSpace: "nowrap" }}>
                              {t.raisedByName || "—"}
                            </div>
                          </td>
                          ...
                          <td style={{ padding: "10px 12px", textAlign: "center", borderRight: "1px solid #e0d8d0" }}>
                            {t.productImage ? (
                              <button onClick={() => setExpandedImage(prev => prev === id ? null : id)}
                                style={{ background: expandedImage === id ? "#dcfce7" : "#f0fdf4", border: "1.5px solid #86efac", borderRadius: 6, padding: "4px 8px", cursor: "pointer", fontSize: 10, fontWeight: 700, color: "#065f46" }}>
                          ...
                          <td style={{ padding: "10px 12px", borderRight: "1px solid #e0d8d0" }}>
                            ...
                            {!["resolved"].includes(s) && (
                              <button onClick={() => setStatusUpdateForm({ show: true, id, note: "", status: "" })}
                                style={{ background: "#5b21b6", color: "white", border: "none", padding: "4px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 600, display: "block", marginTop: 4 }}>
                                📝 Update
                              </button>
                            )}
                          </td>
                          ...
                          <td style={{ padding: "10px 12px" }}>
                            {s !== "resolved" && (
                              <button onClick={() => setResolveForm(p => ({ ...p, [id]: { ...p[id], show: !p[id]?.show } }))}
                                style={{ background: resolveForm[id]?.show ? "#f5f3ff" : "#7c3aed", color: resolveForm[id]?.show ? "#5b21b6" : "white", border: resolveForm[id]?.show ? "1.5px solid #c4b5fd" : "none", padding: "5px 12px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 600 }}>
                                ✅ {resolveForm[id]?.show ? "Cancel" : "Resolve"}
                              </button>
                            )}
                            {s === "resolved" && (
                              <div style={{ fontSize: 11, color: "#059669", fontWeight: 600 }}>✅ Done</div>
                            )}
                          </td>
                        </tr>
                          ...
                          <tr key={`img-${id}`}>
                            <td colSpan={11} style={{ padding: 0, background: "#f0fdf4", borderBottom: "1px solid #86efac" }}>
                              <div style={{ padding: "16px 20px", display: "flex", alignItems: "flex-start", gap: 16, borderLeft: "4px solid #10b981" }}>
                                <img src={t.productImage} alt="Product"
                                  ...
                                  onClick={() => {
                                    ...
                                    win.document.write(`<html><body style="margin:0;background:#111;display:flex;justify-content:center;min-height:100vh;padding:20px;box-sizing:border-box;"><img src="${t.productImage}" style="max-width:100%;height:auto;border-radius:8px;" /></body></html>`);
                                    win.document.close();
                                  }} />
                        ...
                        {resolveForm[id]?.show && s !== "resolved" && (
                          <tr key={`resolve-${id}`} style={{ background: "#faf5ff" }}>
                            <td colSpan={11} style={{ padding: "16px 20px" }}>
                              <div style={{ background: "linear-gradient(135deg,#f5f3ff,#ede9fe)", border: "2px solid #7c3aed", borderRadius: 12, padding: "16px 20px", maxWidth: 600 }}>
                                ...
                                <textarea rows={3} placeholder="Describe what was resolved..."
                                  ...
                                  style={{ width: "100%", padding: "10px 12px", border: "2px solid #c4b5fd", borderRadius: 8, fontSize: 12, outline: "none", fontFamily: "inherit", background: "white", resize: "vertical", color: "#111", lineHeight: 1.5, boxSizing: "border-box" }} />
                                <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
                                  <button onClick={() => handleResolve(id)} disabled={submitting === id}
                                    style={{ background: "linear-gradient(135deg,#7c3aed,#5b21b6)", color: "white", border: "none", padding: "10px 24px", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 800 }}>
                                    {submitting === id ? "⏳ Saving..." : "✅ Confirm Resolve"}
                                  </button>
                                  <button onClick={() => setResolveForm(p => ({ ...p, [id]: { ...p[id], show: false } }))}
                        ...
                        {/* Resolved Info Row */}
                        {s === "resolved" && (
                          <tr key={`res-info-${id}`} style={{ background: "#faf5ff" }}>
