...
const validationErrors = validate();
 ...
 const statusCounts = { all: myTickets.length, open: myTickets.filter(t=>t.status==="open").length, resolved: myTickets.filter(t=>t.status==="resolved").length, rma: myTickets.filter(t=>t.status==="rma").length, reopened: myTickets.filter(t=>t.status==="reopened").length };
    ...
    <div style={{ minHeight:"100vh", background:"#f4f0fa", fontFamily:"DM Sans, sans-serif" }}>
      ...
      {issuePopup && (
        <div onClick={() => setIssuePopup(null)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
         <div onClick={e => e.stopPropagation()} style={{ background:"white", borderRadius:14, padding:"24px 28px", maxWidth:560, width:"100%", maxHeight:"85vh", overflowY:"auto", boxShadow:"0 20px 60px rgba(0,0,0,0.3)", border:"2px solid #c4b5fd", scrollbarWidth:"thin", scrollbarColor:"#7c3aed #f5f3ff" }}>
  ...
  return (
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8, paddingRight: 6, overflowX: "hidden" }}>
        {allHistory.map((h, i) => (
      ...
      <div style={{ background:"linear-gradient(135deg,#7c3aed,#6d28d9)", color:"white", padding:"14px 28px", display:"flex", justifyContent:"space-between", alignItems:"center", boxShadow:"0 2px 12px rgba(124,58,237,0.3)" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <img src="/logo.png" alt="Syrotech" style={{ width:50, height:50, borderRadius:8, objectFit:"contain", background:"rgba(255,255,255,0.15)", padding:2 }} />
          <div>
            <div style={{ fontWeight:700, fontSize:16 }}>Syrotech Networks — Customer Portal</div>
...
<div style={{ marginBottom:20 }}>
  ...
  <div style={{ fontSize:11, whiteSpace:"nowrap", color:"#7c3aed", textDecoration:"underline", textDecorationStyle:"dotted", textDecorationColor:"#c4b5fd" }}>{ticket.model||"—"}</div>
                            ...
                            <td style={tdStyle()}>
                              ...
                              {s==="resolved" && !ticket.feedbackRating && (() => {
