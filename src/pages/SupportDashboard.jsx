...
function sendWhatsAppFeedback(ticket, supportName) {
  ...
  `📞 Support: +91-9870295096 | 9205229997\n` +
...
function sendRMAWhatsApp(ticket, reason) {
 ...
 const message =
  ...
  `• Issue Type       : ${reason}\n\n` +
  `━━━━━━━━━━━━━━━━━━\n🔧 *RMA Service*\n━━━━━━━━━━━━━━━━━━\n` +
  `To find your nearest RMA center, please visit our website:\n` +
  `🌐 *www.syrotech.com*\n\n` +
...
function getAutoNextLevel(allPersons, ticket, currentUserName, allTickets) {
  ...
  const countOpen = (name) =>
    allTickets.filter(t => t.assignTo === name && (t.status === "open" || t.status === "pending")).length;
...
const sent = sendRMAWhatsApp(ticket, rf.reason);
    if (!sent) return;
...
if (!form.model)       e.model       = "Please select an item.";
      ...
      .then(allTicketsData => {
        ...
        if (sameCustomerTicket) {
          ...
          fetch(`${BASE_URL}/tickets/${sameCustomerTicket.id}`, {
            ...
            .then(r => r.json())
            .then(() => {
              setForm({ category: "", subCategory: "", model: "", serialNo: "", mac: "", macPrefix: "", macSuffix: "", customer: "", email: "", phone: "", city: "", state: "", country: "", pincode: "", description: "", assignTo: currentUser?.name || "", productImage: "", raisedVia: "call", issuePrefix: "", issueSuffix: "" });
              setImagePreview(""); setFormErrors({});
             
              setSuccessMsg("✅ Same customer found! Issue updated in existing Ticket. Status set to OPEN.");
  ...
  const activeTotal = counts.resolved + counts.open;
const resolutionPct = activeTotal === 0 ? 0 : Math.round((counts.resolved / activeTotal) * 100);
    ...
    <div style={{ marginBottom: 10 }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #10b981, #059669)", color: "white", padding: "14px 28px", display: "flex", justifyContent: "space-between", alignItems: "center", boxShadow: "0 2px 12px rgba(16,185,129,0.3)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <img src="/GOIPIMAGE2.png" alt="Syrotech" style={{ width: 50, height: 50, borderRadius: 8, objectFit: "contain", background: "white", padding: 2 }} />
          <div>
...
<button onClick={() => setReassignFilter(p => !p)} style={{
                ...
                <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, minWidth: 1100, background: "white" }}>
                  <thead>
                    <tr style={{ background: "linear-gradient(135deg, #059669 0%, #10b981 100%)", position: "sticky", top: 0, zIndex: 2 }}>
                      {["Ticket No","Date","Product","Item Name","Customer","Assigned To","Status","Issue","History","Actions"].map((h, i) => (
  ...
  <div style={{ fontSize: 12, fontWeight: 700, color: "#d97706", textDecoration: "underline", textDecorationStyle: "dotted", textDecorationColor: "#fde68a" }}>{ticket.model || "—"}</div>
                          ...
                          <td style={{ padding: "12px 12px", whiteSpace: "nowrap", borderRight: "1px solid #d1fae5", cursor: "pointer", textAlign: "left" }}
                            onClick={() => setCustomerPopup({ customer: ticket.customer, email: ticket.email, phone: ticket.phone, city: ticket.city, state: ticket.state, country: ticket.country, pincode: ticket.pincode })}>
          ...
          <div style={{ background:"linear-gradient(135deg,#ecfdf5,#d1fae5)", border:"2px solid #10b981", borderRadius:12, padding:"18px 20px", marginTop:4 }}>
            ...
            <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
              <button onClick={() => {
                ...
                sendWhatsAppFeedback(t, currentUser?.name);
...
<div>
  <div style={{ fontSize: 11, fontWeight: 700, color: "#6b7280", textTransform: "uppercase", marginBottom: 6 }}>Additional Note (Optional)</div>
  <input
          ...
          {filtered.length > 0 && (
            <div style={{ overflowX: "scroll", overflowY: "auto", maxHeight: "82vh", borderRadius: 12, border: "1.5px solid #e0d8d0", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
              <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, minWidth: 1200, background: "white" }}>
                <thead>
                  <tr style={{ background: "linear-gradient(135deg, #059669 0%, #10b981 100%)", position: "sticky", top: 0, zIndex: 2 }}>
                   {["Ticket No","Raised From","Raised By","Product / S/N + MAC","Item Name","Customer / KYC","Issue","History","Status","Image","Actions"].map((h, i) => (
